...
function buildAllRedactedBlocks() { 
  ...
  for (let p = 0; p < paragraphs.length; p++) { 
    ...
    for (let i = 0; i < text.length; i++) { 
      ...
      // 🔽 UPDATED: Spaces are now redacted into solid blocks along with letters! 🔽
      if (char === "\n" || char === "\t") { 
        screenArray.push(char); 
      } else { 
        screenArray.push("█"); // Turns letters, punctuation, AND spaces into solid blocks 
      } 
    } 
  } 
